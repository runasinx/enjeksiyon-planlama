...
function handleLogin(e){
  ...
  const account = activeUsers().find(u => u.username === f.username && u.password === f.password);
  if(!account){
...
function renderProducts(){
  ...
  const headers = ['Ürün Kodu','Ürün Adı','Boya Kodu','Kalıp No','Göz','Çevrim sn','Uygun Makineler','İşlem'];
  ...
  tbody.innerHTML = rows.map(p=>`<tr>
    ...
    <td>${escapeHtml(p.boya_kodu || p.boyaKodu || p['Boya Kodu'] || '')}</td>
...
async function editProduct(index){
    ...
    modal.innerHTML = `
      <div class="modal-card plan-edit-card" role="dialog" aria-modal="true">
        <div class="timing-note-head">
          <div><h3>Ürünü Düzenle</h3><p class="note mini-note">Boya kodu dahil ürün bilgilerini güncelleyebilirsiniz.</p></div>
        ...
        <div class="grid two">
          ...
          <label>Boya Kodu<input type="text" data-product-edit-boya placeholder="örn. YM 549" /></label>
  ...
  const boya = modal.querySelector('[data-product-edit-boya]');
  ...
  ad.value = product.ad || '';
  boya.value = product.boya_kodu || product.boyaKodu || product['Boya Kodu'] || '';
  ...
  const onSave = ()=>{
    ...
    product.ad = newAd;
    product.boya_kodu = String(boya.value || '').trim();
    ...
    state.plans.forEach(plan=>{
      if(String(plan['Ürün Kodu'] || '') === String(product.kod || '')){
        ...
        if(!String(plan['Boya Kodu'] || '').trim()) plan['Boya Kodu'] = product.boya_kodu;
      }
    });
...
function filteredProductsForExport(){
  ...
  return state.products
    ...
    .map(p=>({
      'Ürün Kodu': p.kod,
      'Ürün Adı': p.ad,
      'Boya Kodu': p.boya_kodu || p.boyaKodu || p['Boya Kodu'] || '',
      'Kalıp No': p.kalip,
      'Göz': p.goz,
      'Çevrim sn': p.cevrim,
...
$('#productForm')?.addEventListener('submit', e=>{
  ...
  state.products.push({ kod, ad:String(f.ad||'').trim(), boya_kodu:String(f.boya_kodu||'').trim(), kalip:String(f.kalip||'').trim(), goz:num(f.goz), cevrim:num(f.cevrim), machines: machineList.length ? machineList : machines.slice() });
